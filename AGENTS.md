...
Each MCP server configured in `.mcp.json` exposes tools under its server-name prefix.
Agents reference these namespaces in their `tools` list. See `PROJECT.md` `## MCP Servers` for the
full server registry, required env vars, and package details.

| Server name | Tool namespace | Primary consumer(s) |
|---|---|---|
| `playwright-test` | `playwright-test/*` | planner, generator, healer, lead |
| `github` | `github/*` | optional — pr-reviewer, git-workflow (gh CLI preferred) |

No two servers share a namespace prefix. All servers are stdio-based (no port conflicts).
